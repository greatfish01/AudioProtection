...
- **Architecture**:
  ...
  - Fine-tuned layers for multi-class classification.
- **Input**:
  - Preprocessed audio segments converted to embeddings.
- **Output**:
  - Multi-class labels (e.g., `Real`, `WaveNet`, `HiFi-GAN`, etc.).

### **3. Adversarial Model**
- **Purpose**: Generates adversarial examples of audio by adding subtle perturbations designed to challenge and test AI detection systems.
- **Adversarial Audio**:
  - **Attack Method**: Implements a gradient-based adversarial attack using:
    - **FGSM (Fast Gradient Sign Method)**: Perturbs input audio data with calculated noise.
    - **Iterative Method**: Refines perturbations over multiple iterations.
  - **Low-Frequency Noise**: Adds low-frequency sinusoidal noise for additional distortion.
- **Architecture**:
  - Uses Wav2Vec2 for gradient computation on input audio.
  - Generates adversarial waveforms constrained by parameters like `epsilon` (maximum perturbation) and `alpha` (step size).
- **Input**:
