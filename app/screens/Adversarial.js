...
const Adversarial = () => {
    ...
    const uploadAndProcessAudio = async (audioUri, isRecorded = false) => {
        ...
        try {
            ...
            const response = await fetch('http://140.118.145.106:5000/attack', {
                method: 'POST',  
                body: formData,
            ...
            setProcessedAudioUri(result.file_path); // Set processed audio URI from API response

            Alert.alert('Adversarial Attack', 'Adversarial attack added! You can now play the modified audio.');
        } catch (error) {
    ...
    const downloadProcessedAudio = async () => {
        if (!processedAudioUri) {
            Alert.alert('Download Error', 'No processed audio available to download.');
            return;
        }
        try {
            const downloadUri = FileSystem.documentDirectory + 'adversarial_audio.wav';
            await FileSystem.downloadAsync(processedAudioUri, downloadUri);
            Alert.alert('Download Complete', `Audio downloaded to: ${downloadUri}`);
        } catch (error) {
            console.error('Error downloading file:', error);
            Alert.alert('Download Error', 'Failed to download the audio.');
        }
    };
            ...
            <View style={styles.centeredView}>
                ...
                {processedAudioUri && (
                    <View style={styles.actionButtons}>
                        ...
                        <TouchableOpacity style={styles.downloadButton} onPress={downloadProcessedAudio}>
                            <Text style={styles.buttonText}>Download Adversarial Audio</Text>
                        </TouchableOpacity>
                    </View>
                )}
            </View>
...
const styles = StyleSheet.create({
    ...
    button: { backgroundColor: color.ACTIVE_BG, paddingVertical: 12, paddingHorizontal: 20, borderRadius: 5, marginVertical: 5, width: '70%', alignItems: 'center' },
    playButton: { backgroundColor: '#4CAF50', paddingVertical: 12, paddingHorizontal: 20, borderRadius: 5, marginVertical: 5, width: '70%', alignItems: 'center' },
    stopButton: { backgroundColor: '#f44336', paddingVertical: 12, paddingHorizontal: 20, borderRadius: 5, marginVertical: 5, width: '70%', alignItems: 'center' },
    attackButton: { backgroundColor: '#FF9800', paddingVertical: 12, paddingHorizontal: 20, borderRadius: 5, marginVertical: 5, width: '70%', alignItems: 'center' },
    cancelButton: { backgroundColor: '#8b0000', paddingVertical: 12, paddingHorizontal: 20, borderRadius: 5, marginVertical: 5, width: '70%', alignItems: 'center' },
    downloadButton: { backgroundColor: '#2196F3', paddingVertical: 12, paddingHorizontal: 20, borderRadius: 5, marginVertical: 5, width: '70%', alignItems: 'center' },
